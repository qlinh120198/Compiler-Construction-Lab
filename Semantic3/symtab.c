...
void enterBlock(Scope* scope) { // chuyen vao chuong trinh chinh, thay scope hien tai cua bang symtab bang scope cua block chinh
  symtab->currentScope = scope;
}

void exitBlock(void) { // khi thoat khoi 1 block, lien ket pham vi do toi pham vi ben ngoai
  symtab->currentScope = symtab->currentScope->outer; // lien ket toi pham vi ben ngoai
}

// Moi khi duyet den ham hay thu tuc, 1 pham vi moi duoc tao ra, cac khai bao cuc bo se nam trong pham vi nay
// Do do cac bien khai bao cuc bo co the trung ten vs cac bien ben ngoai pham vi do => Check freshIdent,...
void declareObject(Object* obj) {
  if (obj->kind == OBJ_PARAMETER) { // neu la parameter, phai dua vao parameter list cua ham hoac thu tuc chua no
    Object* owner = symtab->currentScope->owner;
    switch (owner->kind) {
    case OBJ_FUNCTION:
      addObject(&(owner->funcAttrs->paramList), obj); // dua vao parameter list cua ham
      break;
    case OBJ_PROCEDURE:
      addObject(&(owner->procAttrs->paramList), obj); // dua vao parameter list cua thu tuc
      break;
    default:
      break;
    }
  ...
  addObject(&(symtab->currentScope->objList), obj); // dua object vao bang ki hieu
}
