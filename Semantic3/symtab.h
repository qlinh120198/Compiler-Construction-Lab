...
struct ParameterAttributes_ { // cau truc tham tri
  enum ParamKind kind;
  Type* type;
...
struct Scope_ {
  ObjectNode *objList; // luu cac doi tuong thuoc cung 1 pham vi
  Object *owner; // doi tuong so huu pham vi (ham, thu tuc, chuong trinh)
  struct Scope_ *outer; // lien ket toi pham vi ben ngoai
};
...
struct SymTab_ {
  ...
  Scope* currentScope; // luu giu pham vi hien tai
  ObjectNode *globalObjectList; // chua cac doi tuong toan cuc
};
